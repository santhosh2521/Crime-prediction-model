...
for district in unique_districts:
    ...
    # Feature engineering
    district_data['target'] = (district_data['ASSAULT ON WOMEN WITH INTENT TO OUTRAGE HER MODESTY'] > 50).astype(int)
    ...
    if len(X) <= 1:
        # Use all available data for training if the dataset is too small
    ...
    # Train the model
    model.fit(X_train_reshaped, y_train, epochs=1, batch_size=32, validation_split=0.2, verbose=0)
    ...
    print(f"Probability of assault in {district}: {probability}")
